...
echo $ANSWER1 |jq -r '.results[].properties | ."ステータス".select.name' | sed -e 's/ToDo/未着手/g' -e 's/InProgress/作業中/g' -e 's/Outsource/依頼中/g' -e 's/Done/完了/g'  > result.txt
...
echo $ANSWER2 |jq -r '.results[].properties | ."ステータス".select.name' | sed -e 's/ToDo/未着手/g' -e 's/InProgress/作業中/g' -e 's/Outsource/依頼中/g' -e 's/Done/完了/g'  >>result.txt
